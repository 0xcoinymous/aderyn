...
contract SF_MintVoucherSignedDeadlineNotChecked040 {
    ...
    function execute(address issuer, address recipient, uint256 tokenId, uint256 deadline, bytes calldata signature) external payable {
        address signer_ = issuer;
        require(ownerOf[tokenId] == address(0), "minted");
        uint256 nonce = nonces[signer_];
        bytes32 digest = keccak256(abi.encode(issuer, recipient, tokenId, nonce, address(this), block.chainid, deadline));
