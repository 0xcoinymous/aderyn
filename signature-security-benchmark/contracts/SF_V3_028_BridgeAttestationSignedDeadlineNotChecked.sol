...
contract SF_BridgeAttestationSignedDeadlineNotChecked028 {
    ...
    function execute(address validator, bytes32 messageId, address recipient, uint256 amount, uint256 deadline, bytes calldata signature) external payable {
        address signer_ = validator;
        require(!released[messageId], "released");
        uint256 nonce = nonces[signer_];
        bytes32 digest = keccak256(abi.encode(validator, messageId, recipient, amount, nonce, address(this), block.chainid, deadline));
