...
contract SF_SignedWithdrawalSafeEpochAndWindowTwoLevel006 {
    ...
    function execute(address account, address recipient, uint256 amount, uint64 epoch, uint256 deadline, bytes calldata signature) external payable {
        address signer_ = account;
        require(credit[account] >= amount, "credit");
        uint256 nonce = nonces[signer_];
        bytes32 digest = _digest(abi.encode(account, recipient, amount, nonce, epoch, deadline));
