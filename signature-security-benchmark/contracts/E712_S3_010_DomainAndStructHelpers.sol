...
contract E712_DomainAndStructHelpers {
    ...
    function _domain() internal view returns(bytes32){return E712_BenchLib.domain("HelperDomain","1",address(this));}
    ...
    function execute(address signer, address recipient, address token, address target, uint256 amount, bytes32 actionId, bytes32 authId, bytes calldata callData, uint8 v, bytes32 r, bytes32 s) external {
        bytes32 digest=E712_BenchLib.typedDataHash(_domain(),_structHash(signer,recipient,token,target,amount,callData,actionId,authId));
