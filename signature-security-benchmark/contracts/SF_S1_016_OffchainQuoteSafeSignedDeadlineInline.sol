...
contract SF_OffchainQuoteSafeSignedDeadlineInline016 {
    ...
    function execute(address quoter, address user, bytes32 quoteId, uint256 amountOut, uint256 deadline, bytes calldata signature) external payable {
        address signer_ = quoter;
        require(!consumedQuote[quoteId], "used");
        uint256 nonce = nonces[signer_];
        bytes32 digest = keccak256(abi.encode(quoter, user, quoteId, amountOut, nonce, address(this), block.chainid, deadline));
