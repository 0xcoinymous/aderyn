...
contract SF_SignedOrderSafeMonotonicTimestampInline004 {
    ...
    function execute(address maker, address taker, uint256 amount, bytes32 orderId, uint256 issuedAt, bytes calldata signature) external payable {
        address signer_ = maker;
        require(!filled[orderId], "filled");
        uint256 nonce = nonces[signer_];
        bytes32 digest = keccak256(abi.encode(maker, taker, amount, orderId, nonce, address(this), block.chainid, issuedAt));
