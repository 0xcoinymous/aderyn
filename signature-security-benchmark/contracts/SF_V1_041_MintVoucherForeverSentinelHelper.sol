...
contract SF_MintVoucherForeverSentinelHelper041 {
    ...
    function execute(address issuer, address recipient, uint256 tokenId, bytes calldata signature) external payable {
        ...
        bytes32 digest = keccak256(abi.encode(issuer, recipient, tokenId, nonce, address(this), block.chainid, type(uint256).max));
