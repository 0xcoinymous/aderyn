...
contract SF_TemporaryApprovalSafeSignedDeadlineInline013 {
    ...
    function execute(address owner, address delegate, uint256 amount, uint256 deadline, bytes calldata signature) external payable {
        ...
        bytes32 digest = keccak256(abi.encode(owner, delegate, amount, nonce, address(this), block.chainid, deadline));
