...
contract E712_PurposeSalt {
    ...
    bytes32 private constant TYPEHASH=keccak256("Action(address signer,address recipient,address token,address target,uint256 amount,bytes32 dataHash,bytes32 actionId,bytes32 authId)"); bytes32 private constant DTYPE=keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract,bytes32 salt)"); bytes32 private constant PURPOSE=keccak256("orders"); function _domain() internal view returns(bytes32){return keccak256(abi.encode(DTYPE,keccak256(bytes("Purpose")),keccak256(bytes("1")),block.chainid,address(this),PURPOSE));}
