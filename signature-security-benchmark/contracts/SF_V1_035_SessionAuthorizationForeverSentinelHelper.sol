...
contract SF_SessionAuthorizationForeverSentinelHelper035 {
    ...
    function execute(address account, address sessionKey, uint256 limit, bytes calldata signature) external payable {
        ...
        bytes32 digest = keccak256(abi.encode(account, sessionKey, limit, nonce, address(this), block.chainid, type(uint256).max));
