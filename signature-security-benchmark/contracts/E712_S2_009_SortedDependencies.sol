...
contract E712_SortedDependencies {
    ...
    bytes32 private constant TYPEHASH=keccak256("Action(address signer,Asset asset,Person person,bytes32 authId)Asset(address token,uint256 amount)Person(address wallet,string name)"); bytes32 private constant ASSET_TYPEHASH=keccak256("Asset(address token,uint256 amount)"); bytes32 private constant PERSON_TYPEHASH=keccak256("Person(address wallet,string name)");
