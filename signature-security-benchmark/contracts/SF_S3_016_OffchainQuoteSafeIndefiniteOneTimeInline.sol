...
contract SF_OffchainQuoteSafeIndefiniteOneTimeInline016 {
    ...
    function execute(address quoter, address user, bytes32 quoteId, uint256 amountOut, bytes calldata signature) external payable {
        ...
        bytes32 digest = keccak256(abi.encode(quoter, user, quoteId, amountOut, nonce, authVersion, address(this), block.chainid));
