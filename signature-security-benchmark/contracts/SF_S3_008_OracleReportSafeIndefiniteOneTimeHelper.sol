...
contract SF_OracleReportSafeIndefiniteOneTimeHelper008 {
    ...
    function execute(address oracle, bytes32 asset, uint256 price, bytes calldata signature) external payable {
        ...
        bytes32 digest = keccak256(abi.encode(oracle, asset, price, nonce, authVersion, address(this), block.chainid));
        require(_verifyFresh(signer_, digest, signature), "fresh/signature");
        nonces[signer_] = nonce + 1;
        archivedPrice[keccak256(abi.encode(asset, price))] = price;
        executionCount += 1;
    }
