...
contract SF_OracleReportUnsignedDeadline022 {
    ...
    function execute(address oracle, bytes32 asset, uint256 price, uint256 deadline, bytes calldata signature) external payable {
        address signer_ = oracle;
        require(price > 0, "price");
        uint256 nonce = nonces[signer_];
        bytes32 digest = keccak256(abi.encode(oracle, asset, price, nonce, address(this), block.chainid));
