...
contract SF_DelegatedExecutionSafeEpochAndWindowTwoLevel015 {
    ...
    function execute(address principal, address target, uint256 value, bytes calldata data, uint64 epoch, uint256 deadline, bytes calldata signature) external payable {
        address signer_ = principal;
        require(target != address(0), "target");
        uint256 nonce = nonces[signer_];
        bytes32 digest = _digest(abi.encode(principal, target, value, keccak256(data), nonce, epoch, deadline));
