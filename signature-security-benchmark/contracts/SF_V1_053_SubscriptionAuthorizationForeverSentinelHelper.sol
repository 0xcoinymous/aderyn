...
contract SF_SubscriptionAuthorizationForeverSentinelHelper053 {
    ...
    function execute(address subscriber, address merchant, uint96 amount, uint32 cycle, bytes calldata signature) external payable {
        ...
        bytes32 digest = keccak256(abi.encode(subscriber, merchant, amount, cycle, nonce, address(this), block.chainid, type(uint256).max));
