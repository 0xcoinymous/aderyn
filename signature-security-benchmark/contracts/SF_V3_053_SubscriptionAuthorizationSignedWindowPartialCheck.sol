...
contract SF_SubscriptionAuthorizationSignedWindowPartialCheck053 {
    ...
    function execute(address subscriber, address merchant, uint96 amount, uint32 cycle, uint256 validAfter, uint256 validBefore, bytes calldata signature) external payable {
        ...
        bytes32 digest = keccak256(abi.encode(subscriber, merchant, amount, cycle, nonce, address(this), block.chainid, validAfter, validBefore));
