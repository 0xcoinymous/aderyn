...
contract E712_TwoLevelHashTyped {
    ...
    function _inner(bytes32 d,bytes32 h) internal pure returns(bytes32){return keccak256(abi.encodePacked("\x19\x01",d,h));} function _outer(bytes32 d,bytes32 h) internal pure returns(bytes32){return _inner(d,h);}
    function execute(address signer, address recipient, address token, address target, uint256 amount, bytes32 actionId, bytes32 authId, bytes calldata callData, uint8 v, bytes32 r, bytes32 s) external {
        ...
        bytes32 digest=_outer(domainSeparator,structHash);
