...
contract SF_OffchainQuoteSignedWindowPartialCheck047 {
    ...
    function execute(address quoter, address user, bytes32 quoteId, uint256 amountOut, uint256 validAfter, uint256 validBefore, bytes calldata signature) external payable {
        ...
        bytes32 digest = keccak256(abi.encode(quoter, user, quoteId, amountOut, nonce, address(this), block.chainid, validAfter, validBefore));
