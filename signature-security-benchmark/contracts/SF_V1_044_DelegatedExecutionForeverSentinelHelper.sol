...
contract SF_DelegatedExecutionForeverSentinelHelper044 {
    ...
    function execute(address principal, address target, uint256 value, bytes calldata data, bytes calldata signature) external payable {
        ...
        bytes32 digest = keccak256(abi.encode(principal, target, value, keccak256(data), nonce, address(this), block.chainid, type(uint256).max));
