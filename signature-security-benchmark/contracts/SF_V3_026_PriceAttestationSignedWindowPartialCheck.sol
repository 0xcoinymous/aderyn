...
contract SF_PriceAttestationSignedWindowPartialCheck026 {
    ...
    function execute(address attestor, address asset, uint256 price, uint32 confidence, uint256 validAfter, uint256 validBefore, bytes calldata signature) external payable {
        address signer_ = attestor;
        require(confidence > 0, "confidence");
        uint256 nonce = nonces[signer_];
        bytes32 digest = keccak256(abi.encode(attestor, asset, price, confidence, nonce, address(this), block.chainid, validAfter, validBefore));
