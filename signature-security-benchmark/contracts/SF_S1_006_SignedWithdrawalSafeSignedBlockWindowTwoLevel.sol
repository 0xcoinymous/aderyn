...
contract SF_SignedWithdrawalSafeSignedBlockWindowTwoLevel006 {
    ...
    function execute(address account, address recipient, uint256 amount, uint256 validFromBlock, uint256 validThroughBlock, bytes calldata signature) external payable {
        address signer_ = account;
        require(credit[account] >= amount, "credit");
        uint256 nonce = nonces[signer_];
        bytes32 digest = _digest(abi.encode(account, recipient, amount, nonce, validFromBlock, validThroughBlock));
