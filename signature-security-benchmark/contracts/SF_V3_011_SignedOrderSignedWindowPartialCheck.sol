...
contract SF_SignedOrderSignedWindowPartialCheck011 {
    ...
    function execute(address maker, address taker, uint256 amount, bytes32 orderId, uint256 validAfter, uint256 validBefore, bytes calldata signature) external payable {
        ...
        bytes32 digest = keccak256(abi.encode(maker, taker, amount, orderId, nonce, address(this), block.chainid, validAfter, validBefore));
