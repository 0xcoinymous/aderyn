...
contract SF_OracleReportSignedTimestampUnitMismatch024 {
    ...
    function execute(address oracle, bytes32 asset, uint256 price, uint256 issuedAtMs, bytes calldata signature) external payable {
        address signer_ = oracle;
        require(price > 0, "price");
        uint256 nonce = nonces[signer_];
        bytes32 digest = _digest(abi.encode(oracle, asset, price, nonce, issuedAtMs));
