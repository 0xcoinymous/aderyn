...
contract SF_SubscriptionAuthorizationSafeIndefiniteRevocableVersion018 {
    ...
    function execute(address subscriber, address merchant, uint96 amount, uint32 cycle, bytes calldata signature) external payable {
        ...
        bytes32 digest = _digest(abi.encode(subscriber, merchant, amount, cycle, nonce, authVersion));
