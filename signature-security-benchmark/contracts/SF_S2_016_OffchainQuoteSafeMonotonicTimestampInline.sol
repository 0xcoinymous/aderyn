...
contract SF_OffchainQuoteSafeMonotonicTimestampInline016 {
    ...
    function execute(address quoter, address user, bytes32 quoteId, uint256 amountOut, uint256 issuedAt, bytes calldata signature) external payable {
        ...
        bytes32 digest = keccak256(abi.encode(quoter, user, quoteId, amountOut, nonce, address(this), block.chainid, issuedAt));
