...
contract SF_SubscriptionAuthorizationSignedDeadlineNotChecked052 {
    ...
    function execute(address subscriber, address merchant, uint96 amount, uint32 cycle, uint256 deadline, bytes calldata signature) external payable {
        address signer_ = subscriber;
        require(merchant != address(0), "merchant");
        uint256 nonce = nonces[signer_];
        bytes32 digest = keccak256(abi.encode(subscriber, merchant, amount, cycle, nonce, address(this), block.chainid, deadline));
