...
contract E712_SaltNoVerifier {
    ...
    bytes32 private constant TYPEHASH=keccak256("Action(address signer,address recipient,address token,address target,uint256 amount,bytes32 dataHash,bytes32 actionId,bytes32 authId)"); bytes32 private constant DTYPE=keccak256("EIP712Domain(string name,string version,uint256 chainId,bytes32 salt)"); bytes32 private constant SALT=keccak256("shared-protocol-validator");
    function execute(address signer, address recipient, address token, address target, uint256 amount, bytes32 actionId, bytes32 authId, bytes calldata callData, uint8 v, bytes32 r, bytes32 s) external {
        ...
        bytes32 digest=E712_BenchLib.typedDataHash(keccak256(abi.encode(DTYPE,keccak256(bytes("SharedProtocol")),keccak256(bytes("1")),block.chainid,SALT)),structHash);
