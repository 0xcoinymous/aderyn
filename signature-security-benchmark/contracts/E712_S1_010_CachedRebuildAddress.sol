...
contract E712_CachedRebuildAddress {
    ...
    bytes32 private constant TYPEHASH=keccak256("Action(address signer,address recipient,address token,address target,uint256 amount,bytes32 dataHash,bytes32 actionId,bytes32 authId)"); address private immutable cachedThis; bytes32 private immutable cachedDomain; constructor(){cachedThis=address(this); cachedDomain=E712_BenchLib.domain("AddressCache","1",address(this));} function _domain() internal view returns(bytes32){return address(this)==cachedThis?cachedDomain:E712_BenchLib.domain("AddressCache","1",address(this));}
