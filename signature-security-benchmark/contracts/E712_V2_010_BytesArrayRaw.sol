...
contract E712_BytesArrayRaw {
    ...
    bytes32 private constant TYPEHASH = keccak256("Action(address signer,bytes[] calls,bytes32 actionId,bytes32 authId)");
    function execute(address signer, address recipient, address token, address target, uint256 amount, bytes32 actionId, bytes32 authId, bytes calldata callData, uint8 v, bytes32 r, bytes32 s, bytes[] calldata calls) external {
        bytes32 structHash=keccak256(abi.encode(TYPEHASH,signer,keccak256(abi.encode(calls)),actionId,authId));
