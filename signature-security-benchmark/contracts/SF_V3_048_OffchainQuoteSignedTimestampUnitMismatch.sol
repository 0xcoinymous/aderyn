...
contract SF_OffchainQuoteSignedTimestampUnitMismatch048 {
    ...
    function execute(address quoter, address user, bytes32 quoteId, uint256 amountOut, uint256 issuedAtMs, bytes calldata signature) external payable {
        address signer_ = quoter;
        require(!consumedQuote[quoteId], "used");
        uint256 nonce = nonces[signer_];
        bytes32 digest = _digest(abi.encode(quoter, user, quoteId, amountOut, nonce, issuedAtMs));
