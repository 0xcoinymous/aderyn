...
contract E712_MissingDependency {
    ...
    bytes32 private constant TYPEHASH = keccak256("Action(address signer,Asset asset,bytes32 actionId,bytes32 authId)");
    function execute(address signer, address recipient, address token, address target, uint256 amount, bytes32 actionId, bytes32 authId, bytes calldata callData, uint8 v, bytes32 r, bytes32 s, Asset calldata asset) external {
        bytes32 structHash=keccak256(abi.encode(TYPEHASH,signer,keccak256(abi.encode(ASSET_TYPEHASH,asset.token,asset.amount)),actionId,authId));
