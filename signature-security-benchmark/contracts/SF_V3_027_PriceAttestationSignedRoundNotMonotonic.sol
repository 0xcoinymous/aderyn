...
contract SF_PriceAttestationSignedRoundNotMonotonic027 {
    ...
    function execute(address attestor, address asset, uint256 price, uint32 confidence, uint64 round, bytes calldata signature) external payable {
        address signer_ = attestor;
        require(confidence > 0, "confidence");
        uint256 nonce = nonces[signer_];
        bytes32 digest = _digest(abi.encode(attestor, asset, price, confidence, nonce, round));
