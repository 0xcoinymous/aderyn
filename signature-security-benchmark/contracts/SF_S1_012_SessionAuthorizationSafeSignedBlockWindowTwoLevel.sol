...
contract SF_SessionAuthorizationSafeSignedBlockWindowTwoLevel012 {
    ...
    function execute(address account, address sessionKey, uint256 limit, uint256 validFromBlock, uint256 validThroughBlock, bytes calldata signature) external payable {
        address signer_ = account;
        require(sessionKey != address(0), "key");
        uint256 nonce = nonces[signer_];
        bytes32 digest = _digest(abi.encode(account, sessionKey, limit, nonce, validFromBlock, validThroughBlock));
